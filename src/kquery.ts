...
class K$ extends Array<HTMLElement> {
    ...
    constructor(args: any) {
        ...
        if (args.length === 1 && Array.isArray(args[0]))
            args[0].forEach((i) => this.push(i), this);
        else
            K$.convertArgumentsToElementsArray(args, window.document).forEach((i) => i instanceof HTMLElement ? this.push(i) : null, this);
    }
    ...
    static convertArgumentsToElementsArray(args: any[], elementParent: ParentNode): Node[] {
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (a instanceof HTMLElement || a instanceof DocumentFragment)
                ...
                if (newCollection.length > 0)
                    result = result.concat(newCollection);
            }
    ...
    static asK$(...args: any): K$ {
        ...
        if (args.length === 1 && typeof args[0] === "object" && (args[0] instanceof K$ /* || (args[0] instanceof Array && args[0].isK$) */))
            return args[0];
    ...
    static getDiv(innerHTML?: string, classes?: string): HTMLDivElement {
        ...
        if (classes)
            classes.split(' ').forEach((c) => divElement.classList.add(c));
        if (innerHTML)
    ...
    val(v: any) {
        if (v !== undefined) {
            ...
            if (this.first instanceof HTMLInputElement)
    ...
    append(element: HTMLElement | K$ | HTMLElement[]) {
        ...
        if (element instanceof K$ || element instanceof Array)
            K$.iterate(element, (e) => this.first?.appendChild(e));
        else
            this.first.appendChild(element);
    ...
    prepend(element: HTMLElement | K$ | HTMLElement[]) {
        ...
        if (element instanceof K$ || element instanceof Array)
            ...
            this.first.insertBefore(element, this.first.firstElementChild);
...
export function $(...args: any) { return K$.asK$(...args); }
